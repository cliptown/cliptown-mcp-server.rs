...
#
# Multi-stage image for cliptown-mcp-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t cliptown-mcp-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" cliptown-mcp-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=cliptown-mcp-server-target-${TARGETARCH},sharing=locked \
    cargo build --release --locked --bin cliptown-mcp-server \
    && strip "target/release/cliptown-mcp-server" \
    && cp "target/release/cliptown-mcp-server" "/usr/local/bin/cliptown-mcp-server"
...
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/cliptown-mcp-server"]
